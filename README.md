...
- For `.pt` sources, CDS now emits a second ONNX specifically for RKNN at `artifacts/models/<run-id>/exports/best.rknn.onnx`. It keeps decoded `boxes` and `scores` in separate outputs for quantization, while the normal `best.onnx` export remains unchanged for generic use.
